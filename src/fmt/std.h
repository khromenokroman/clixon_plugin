...
auto write_demangled_name(OutputIt out, const std::type_info& ti) -> OutputIt {
    ...
    int status = 0;
    std::size_t size = 0;
    std::unique_ptr<char, void (*)(void*)> demangled_name_ptr(abi::__cxa_demangle(ti.name(), nullptr, &size, &status),
                                                              &std::free);
...
struct formatter<std::complex<T>, Char> {
   private:
    ...
    FMT_CONSTEXPR auto do_format(const std::complex<T>& c, detail::dynamic_format_specs<Char>& specs,
