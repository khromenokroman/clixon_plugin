...
struct is_back_insert_iterator<
    It, bool_constant<std::is_same<decltype(adl::invoke_back_inserter<typename It::container_type>()), It>::value>>
    : std::true_type {};
...
class basic_string_view {
   ...
   public:
    ...
    template <typename S,
              FMT_ENABLE_IF(detail::is_std_string_like<S>::value&& std::is_same<typename S::value_type, Char>::value)>
...
template <typename OutputIt>
class iterator_buffer<OutputIt, enable_if_t<detail::is_back_insert_iterator<OutputIt>::value &&
...
constexpr auto has_const_formatter_impl(T*) -> decltype(typename Context::template formatter_type<T>().format(
...
struct arg_mapper {
    ...
    template <typename T, typename U = remove_const_t<T>,
              FMT_ENABLE_IF((std::is_class<U>::value || std::is_enum<U>::value || std::is_union<U>::value ||
                            ...
                            !has_to_string_view<U>::value && !is_char<U>::value && !is_named_arg<U>::value &&
                            !std::is_integral<U>::value && !std::is_arithmetic<format_as_t<U>>::value)>
...
class basic_format_arg {
   ...
   public:
    ...
    auto format_custom(const char_type* parse_begin, typename Context::parse_context_type& parse_ctx,
                       Context& ctx) -> bool {
...
using is_formattable =
    bool_constant<!std::is_base_of<detail::unformattable, decltype(detail::arg_mapper<buffered_context<Char>>().map(
                                                              std::declval<T&>()))>::value>;
...
template <typename Context = format_context, typename... T, size_t NUM_ARGS = sizeof...(T),
...
template <typename Char>
using unsigned_char =
    typename conditional_t<std::is_integral<Char>::value, std::make_unsigned<Char>, type_identity<unsigned>>::type;
...
FMT_CONSTEXPR auto parse_nonnegative_int(const Char*& begin, const Char* end, int error_value) noexcept -> int {
    ...
    return num_digits == digits10 + 1 && prev * 10ull + unsigned(p[-1] - '0') <= max ? static_cast<int>(value)
                                                                                     : error_value;
}
...
template <typename Char>
FMT_CONSTEXPR auto parse_format_specs(const Char* begin, const Char* end, dynamic_format_specs<Char>& specs,
...
template <typename Char>
void vformat_to(buffer<Char>& buf, basic_string_view<Char> fmt, typename vformat_args<Char>::type args,
                locale_ref loc = {});
